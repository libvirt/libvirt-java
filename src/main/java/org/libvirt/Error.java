...
public class Error implements Serializable {
     ...
     * {@code values.length}.
     *
     * @return n-th item of {@code values} when {@code n < values.length},
     * otherwise the last item of {@code values}.
     */
    ...
    public String toString() {
        return String.format("level:%s%ncode:%s%ndomain:%s%nhasConn:%b%nhasDom:%b%nhasNet:%b%nmessage:%s%nstr1:%s%nstr2:%s%nstr3:%s%nint1:%d%nint2:%d%n", level, code, domain, hasConn(), hasDom(),
                hasNet(), message, str1, str2, str3, int1, int2);
    }
}
