...
public class Connect {
	...
	public static Connect connectionForDomain(Domain domain) {
		ConnectionPointer conn = Libvirt.INSTANCE
				.virDomainGetConnect(domain.VDP);
	...
	public static Connect connectionForNetwork(Network network) {
		ConnectionPointer conn = Libvirt.INSTANCE
				.virNetworkGetConnect(network.VNP);
		return new Connect(conn);
	}
	...
	public String[] listDevices(String capabilityName) throws LibvirtException {
		...
		if (maxDevices > 0) {
			libvirt.virNodeListDevices(VCP, capabilityName, names, maxDevices,
					0);
			processError();
		}
		return names;
	...
	public long getCellsFreeMemory(int startCells, int maxCells)
		...
		libvirt.virNodeGetCellsFreeMemory(VCP, returnValue, startCells,
				maxCells);
		processError();
	...
	public StoragePool storagePoolCreateXML(String xmlDesc, int flags)
			throws LibvirtException {
		StoragePoolPointer ptr = libvirt.virStoragePoolCreateXML(VCP, xmlDesc,
				flags);
		processError();
	...
	public StoragePool storagePoolDefineXML(String xml, int flags)
			throws LibvirtException {
		StoragePoolPointer ptr = libvirt.virStoragePoolDefineXML(VCP, xml,
				flags);
		processError();
		return new StoragePool(this, ptr);
	}
	...
	public StoragePool storagePoolLookupByUUID(int[] UUID)
		...
		StoragePoolPointer ptr = libvirt.virStoragePoolLookupByUUID(VCP,
				uuidBytes);
		processError();
		if (ptr != null) {
	...
	public StoragePool storagePoolLookupByUUIDString(String UUID)
			throws LibvirtException {
		StoragePool returnValue = null;
		StoragePoolPointer ptr = libvirt.virStoragePoolLookupByUUIDString(VCP,
				UUID);
		processError();
		if (ptr != null) {
			returnValue = new StoragePool(this, ptr);
	...
	// TODO Post 0.5.1
	// /**
	// * Reads native configuration data describing a domain, and generates
	// libvirt domain XML.
	// * The format of the native data is hypervisor dependant.
	// *
	// * @param nativeFormat configuration format exporting to
	// * @param domainXML the domain confid to export
	// * @param flags
	// * @return native config datafil
	// */
	...
	// nativeFormat, nativeConfig, flags) ;
	// processError() ;
	// return returnValue ;
	// }
